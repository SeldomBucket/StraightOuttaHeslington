...
public class UIEquipment extends UIComponent {
    ...
    public void render(SpriteBatch batch, NinePatch patch) {
        ...
        switch (equipable.getType()) {
            case HEAD:
                renderText (batch, "HEAD: " + equipable.getName(), x, y + (LINE_HEIGHT / 2), Color.WHITE);
                renderModifiers(batch);
                break;
            case CHEST:
                renderText (batch, "CHEST: " + equipable.getName(), x, y + (LINE_HEIGHT / 2), Color.WHITE);
                renderModifiers(batch);
                break;
            case FEET:
                renderText (batch, "FEET: " + equipable.getName(), x, y + (LINE_HEIGHT / 2), Color.WHITE);
                renderModifiers(batch);
                break;
            case WEAPON:
                renderText (batch, "WEAPON: " + equipable.getName(), x, y + (LINE_HEIGHT / 2), Color.WHITE);
                renderModifiers(batch);
                break;
            case ACCESSORY:
                renderText (batch, "ACCESSORY: " + equipable.getName(), x, y + (LINE_HEIGHT / 2), Color.WHITE);
                renderModifiers(batch);
                break;
            default:
                renderText (batch, "EMPTY SLOT", x, y + (LINE_HEIGHT / 2), Color.WHITE);
                break;
        }
    }
    ...
    private void renderModifiers(SpriteBatch batch) {
        ...
        renderText (batch, "SPD:" + modifiers[0], x, y - (LINE_HEIGHT / 2), Color.WHITE);
        renderText (batch, "STR:" + modifiers[1], x + 80, y - (LINE_HEIGHT / 2), Color.WHITE);
        renderText (batch, "DEX:" + modifiers[2], x + 160, y - (LINE_HEIGHT / 2), Color.WHITE);
        renderText (batch, "INT:" + modifiers[3], x + 240, y - (LINE_HEIGHT / 2), Color.WHITE);
        renderText (batch, "ARM:" + modifiers[4], x + 320, y - (LINE_HEIGHT / 2), Color.WHITE);
        renderText (batch, equipable.getDescription(), x, y - (LINE_HEIGHT * 3 / 2), Color.LIGHT_GRAY);
    }
